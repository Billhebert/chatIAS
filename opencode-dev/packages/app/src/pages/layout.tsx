...
import { useUserSessions } from "@/context/user-sessions"
...
export default function Layout(props: ParentProps) {
    ...
    async function pollUpdate() {
      ...
      if (updateAvailable && toastId === undefined) {
        toastId = showToast({
          ...
          description: `Uma nova versão do Suatec (${version}) está disponível para instalar.`,
          actions: [
            {
  ...
  onMount(() => {
    ...
    const unsub = globalSDK.event.listen((e) => {
      ...
      const description = `${sessionTitle} em ${projectName} precisa de permissão`
      ...
      void platform.notify("Permissão necessária", description, href)
  ...
  command.register(() => {
    ...
    for (const [id, definition] of availableThemeEntries()) {
      commands.push({
        id: `theme.set.${id}`,
        title: `Usar tema: ${definition.name ?? id}`,
        category: "Tema",
    ...
    for (const scheme of colorSchemeOrder) {
      commands.push({
        ...
        title: `Usar esquema de cores: ${colorSchemeLabel[scheme]}`,
        category: "Tema",
        onSelect: () => theme.commitPreview(),
        onHighlight: () => {
            ...
            >
              <div class="flex items-center self-stretch gap-6 justify-between transition-[padding] group-hover/session:pr-7 group-focus-within/session:pr-7 group-active/session:pr-7">
                ...
                <div class="shrink-0 group-hover/session:hidden group-active/session:hidden group-focus-within/session:hidden">
                  <Switch>
                    ...
                    <Match when={true}>
                      <span class="text-12-regular text-text-subtle text-right whitespace-nowrap">
              ...
              <Show when={props.session.summary?.files}>
                <div class="flex justify-between items-center self-stretch">
                  <span class="text-12-regular text-text-subtle">{`${props.session.summary?.files || "Nenhum"} arquivo${props.session.summary?.files !== 1 ? "s" : ""} alterado${props.session.summary?.files !== 1 ? "s" : ""}`}</span>
                  <Show when={props.session.summary}>{(summary) => <DiffChanges changes={summary()} />}</Show>
                </div>
              </Show>
            </A>
          </Tooltip>
          <Show when={auth.hasPermission("canArchiveSessions")}>
  ...
  const SortableProject = (props: { project: LocalProject; mobile?: boolean }): JSX.Element => {
    ...
    const sessions = createMemo(() =>
      userSessions.filterUserSessions(
        stores()
          .flatMap((store) => store.session.filter((session) => session.directory === store.path.directory))
          .toSorted(sortSessions)
      ),
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenu.Portal>
                      <DropdownMenu.Content>
                        ...
                        >
                          <DropdownMenu.ItemLabel>Editar projeto</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                        <DropdownMenu.Item onSelect={() => closeProject(props.project.worktree)}>
                          <DropdownMenu.ItemLabel>Fechar projeto</DropdownMenu.ItemLabel>
                        </DropdownMenu.Item>
                      </DropdownMenu.Content>
                    </DropdownMenu.Portal>
                  </DropdownMenu>
                  <TooltipKeybind placement="top" title="Nova sessão" keybind={command.keybind("session.new")}>
                    ...
                    >
                      <div class="flex items-center self-stretch w-full">
                        <div class="flex-1 min-w-0">
                          <Tooltip placement={props.mobile ? "bottom" : "right"} value="Nova sessão">
                            <A
      ...
      <div class="flex flex-col self-stretch h-full items-center justify-between overflow-hidden min-h-0">
        <div class="flex flex-col items-start self-stretch gap-4 min-h-0">
          <Show when={!sidebarProps.mobile}>
            ...
            >
              <A href="/" class="shrink-0 h-8 flex items-center justify-start px-2 w-full overflow-hidden" data-tauri-drag-region>
                <Show when={expanded()} fallback={<span class="text-lg font-bold text-text-strong">S</span>}>
                  <span class="text-lg font-bold text-text-strong whitespace-nowrap">SUATEC - AGENTE</span>
                </Show>
              </A>
            </div>
          </Show>
        ...
        <div class="flex flex-col gap-1.5 self-stretch items-start shrink-0 px-2 py-3">
            ...
            <Match when={providers.all().length > 0 && !providers.paid().length && expanded()}>
              <div class="rounded-md bg-background-stronger shadow-xs-border-base">
                <div class="p-3 flex flex-col gap-2">
                  <div class="text-12-medium text-text-strong">Primeiros passos</div>
                  <div class="text-text-base">O Suatec inclui modelos gratuitos para você começar imediatamente.</div>
                  <div class="text-text-base">Conecte qualquer provedor para usar modelos, incluindo Claude, GPT, Gemini etc.</div>
                </div>
                <Tooltip placement="right" value="Conectar provedor" inactive={expanded()}>
                  <Button
            ...
            <Match when={providers.all().length > 0}>
              <Tooltip placement="right" value="Conectar provedor" inactive={expanded()}>
                <Button
                  ...
                  onClick={connectProvider}
                >
                  <Show when={expanded()}>Conectar provedor</Show>
                </Button>
              </Tooltip>
            </Match>
          </Switch>
          ...
          >
            ...
            >
              <Show when={expanded()}>Abrir projeto</Show>
            </Button>
          </Tooltip>
          <Tooltip placement="right" value="Enviar feedback" inactive={expanded()}>
            <Button
              as={"a"}
              ...
              icon="bubble-5"
            >
              <Show when={expanded()}>Enviar feedback</Show>
            </Button>
          </Tooltip>
          <div class="w-full border-t border-border-weak-base my-1" />
          <Tooltip placement="right" value={auth.isDeveloperMode() ? "Modo Desenvolvedor ATIVO" : "Modo Padrão"} inactive={expanded()}>
            <Button
              class="flex w-full text-left justify-start text-text-base stroke-[1.5px] rounded-lg px-2"
              variant="ghost"
              size="large"
              icon={auth.isDeveloperMode() ? "terminal" : "user"}
              onClick={async () => {
                await auth.toggleDeveloperMode()
                showToast({
                  title: auth.isDeveloperMode() ? "Modo Desenvolvedor" : "Modo Padrão",
                  description: auth.isDeveloperMode() 
                    ? "Interface completa com terminal e detalhes técnicos" 
                    : "Interface simplificada",
                })
              }}
            >
              <Show when={expanded()}>
                <span classList={{ "text-text-interactive-base": auth.isDeveloperMode() }}>
                  {auth.isDeveloperMode() ? "Modo Desenvolvedor" : "Modo Padrão"}
                </span>
              </Show>
            </Button>
          </Tooltip>
<Show when={auth.isMaster() || auth.isAdmin()}>
            <Tooltip placement="right" value="Administração" inactive={expanded()}>
              <Button
                ...
                icon="settings"
              >
                <Show when={expanded()}>Administração</Show>
              </Button>
            </Tooltip>
          </Show>
          ...
          <Show when={auth.currentUser()}>
            <div class="flex w-full items-center justify-between px-2">
              ...
              <Tooltip placement="right" value={`Sair (${auth.currentUser()?.username})`} inactive={expanded()}>
                <Button
                  class="flex text-left justify-start text-text-base stroke-[1.5px] rounded-lg px-2"
                  variant="ghost"
                  size="large"
                  icon="arrow-left"
                  onClick={() => {
        ...
        <div class="xl:hidden">
          ...
          >
            <div class="border-b border-border-weak-base w-full h-12 ml-px flex items-center pl-1.75 shrink-0">
              ...
              >
                <span class="text-lg font-bold text-text-strong">SUATEC - AGENTE</span>
              </A>
            </div>

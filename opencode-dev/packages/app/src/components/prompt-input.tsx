...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    if (!session && isNewSession) {
      ...
      if (session) {
        // Registra a sessao para o usuario atual
        userSessions.registerSession(session.id)
    ...
    // SUATEC: Ensure parts have correct format for server
    // The server expects TextPart with "text" property, not "content"
    console.log("[SUATEC DEBUG] requestParts BEFORE conversion:", JSON.stringify(requestParts))
    const serverParts = requestParts.map((part: any) => {
      if (part.type === "text") {
        return {
          type: "text" as const,
          text: part.text ?? part.content ?? "",  // Use text first, fallback to content
        }
      }
      if (part.type === "file") {
        return {
    ...
    console.log("[SUATEC DEBUG] serverParts AFTER conversion:", JSON.stringify(serverParts))

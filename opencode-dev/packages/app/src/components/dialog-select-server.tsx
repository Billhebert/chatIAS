...
export function DialogSelectServer() {
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    if (!result.healthy) {
      setStore("error", "Não foi possível conectar ao servidor")
      return
    }
  ...
  return (
    <Dialog title="Servidores" description="Selecione qual servidor o app deve conectar.">
      <div class="flex flex-col gap-4 pb-4">
        <List
          search={{ placeholder: "Buscar servidores", autofocus: true }}
        ...
        <div class="mt-6 px-3 flex flex-col gap-1.5">
          <div class="px-3">
            <h3 class="text-14-regular text-text-weak">Adicionar servidor</h3>
          </div>
          <form onSubmit={handleSubmit}>
            <div class="flex items-start gap-2">
              ...
              <Button type="submit" variant="secondary" icon="plus-small" size="large" disabled={store.adding}>
                {store.adding ? "Verificando..." : "Adicionar"}
              </Button>
            </div>
          </form>
        </div>
        ...
        <Show when={isDesktop}>
          <div class="mt-6 px-3 flex flex-col gap-1.5">
            <div class="px-3">
              <h3 class="text-14-regular text-text-weak">Servidor padrão</h3>
              <p class="text-12-regular text-text-weak mt-1">
                Conectar a este servidor ao iniciar o app em vez de iniciar um servidor local. Requer reinicialização.
              </p>
            </div>
            <div class="flex items-center gap-2 px-3 py-2">
              <Show
                ...
                fallback={
                  <Show
                    when={server.url}
                    fallback={<span class="text-14-regular text-text-weak">Nenhum servidor selecionado</span>}
                  >
                    <Button
                      variant="secondary"
                      size="small"
                    ...
                    >
                      Definir servidor atual como padrão
                    </Button>
                  </Show>
                }
              >

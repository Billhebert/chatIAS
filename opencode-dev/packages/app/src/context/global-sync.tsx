...
function createGlobalSync() {
  ...
  async function loadSessions(directory: string) {
    ...
    globalSDK.client.session
      ...
      .catch((err) => {
        ...
        showToast({ title: `Falha ao carregar sessões para ${project}`, description: err.message })
      })
  }
  ...
  async function bootstrap() {
    console.log("[SUATEC DEBUG] Tentando conectar ao servidor:", globalSDK.url)
    const health = await globalSDK.client.global
      .health()
      .then((x) => {
        console.log("[SUATEC DEBUG] Resposta do servidor:", x)
        return x.data
      })
      .catch((err) => {
        console.error("[SUATEC DEBUG] Erro ao conectar:", err)
        return undefined
      })
    if (!health?.healthy) {
      setGlobalStore(
        "error",
        new Error(`Não foi possível conectar ao servidor de processamento. O serviço pode estar temporariamente indisponível.`),
      )
      return
    }

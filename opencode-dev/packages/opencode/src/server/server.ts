...
export namespace Server {
  ...
  // SUATEC: Helper para extrair userId do token de autenticação
  async function getUserIdFromRequest(c: any): Promise<string | undefined> {
    try {
  ...
  // SUATEC: Helper para verificar se usuário é admin
  async function isAdminFromRequest(c: any): Promise<boolean> {
    try {
      const authHeader = c.req.header("Authorization")
      ...
      app
        ...
        .get(
          ...
          async (c) => {
            ...
            // SUATEC: Filtrar sessões por usuário
            const userId = await getUserIdFromRequest(c)
            const isAdmin = await isAdminFromRequest(c)
            
            for await (const session of Session.list()) {
              // Verificar acesso do usuário à sessão
              if (userId && !Session.canAccessSession(session, userId, isAdmin)) continue
        ...
        .post(
          ...
          async (c) => {
            ...
            // SUATEC: Incluir userId do usuário autenticado na sessão
            const userId = await getUserIdFromRequest(c)
            ...
            const session = await Session.create({ ...body, userId })
            
            console.log("[SESSION CREATE] Session created", {
          ...
          async (c) => {
            ...
            return stream(c, async (stream) => {
              try {
                ...
                console.error("[MESSAGE] Error processing message:", err.message, err.stack)
  ...
  export function listen(opts: { port: number; hostname: string; mdns?: boolean; cors?: string[] }) {
    ...
    // Initialize User module (connects to PostgreSQL if enabled)
    User.initialize().catch((err) => {
      log.warn("User module initialization failed", { error: err })
    })

    // Initialize RAG module (connects to Qdrant if available)
    RAG.initialize().catch((err) => {
      log.warn("RAG module initialization failed", { error: err })
    })

...
import { transformSystemWithRAG } from "@/rag/integration"
import { Session } from "."
...
export namespace LLM {
  ...
  export async function stream(input: StreamInput) {
    ...
    // SUATEC: Inject RAG context into system prompt
    try {
      const session = await Session.get(input.sessionID)
      l.info("RAG: checking session", { 
      ...
      if (session?.userId) {
        // Extract text from the last user message in the messages array
        ...
        l.info("RAG: extracted user text", { 
        ...
        if (userText) {
          ...
          l.info("RAG: transformSystemWithRAG completed", { systemLength: system.length })
        }
      } else {
        l.info("RAG: no userId in session, skipping")
      }
    } catch (err: any) {
      log.warn("Failed to inject RAG context", { error: err.message, stack: err.stack })
    }

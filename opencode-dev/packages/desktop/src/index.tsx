...
// Safe ostype that returns "browser" when not in Tauri
const safeOsType = () => {
...
const platform: Platform = {
  ...
  update: async () => {
    ...
    if (safeOsType() === "windows") await invoke("kill_sidecar").catch(() => undefined)
...
// Only create menu when inside Tauri
if (isTauri()) {
  createMenu()
}
...
function ServerGate(props: ParentProps) {
  const [status] = createResource(async () => {
    // Skip server check if not in Tauri (browser dev mode)
    if (!isTauri()) return

...
class BaseAgent {
  constructor(name, description, config = {}) {
    ...
    this.allowedSubagents = config.allowedSubagents || [];
  }

  /**
   * Verifica se o agente tem permissão para usar uma tool
   */
  canUseTool(toolName) {
    // Se allowedTools estiver vazio, permite tudo (backward compatibility)
    if (this.allowedTools.length === 0) return true;
    return this.allowedTools.includes(toolName);
  }

  /**
   * Verifica se o agente tem permissão para chamar um subagente
   */
  canCallSubagent(subagentName) {
    // Se allowedSubagents estiver vazio, permite tudo (backward compatibility)
    if (this.allowedSubagents.length === 0) return true;
    return this.allowedSubagents.includes(subagentName);
  }
  ...
  /**
   * Chama um subagente (com validação de permissão)
   */
  async callSubagent(name, input) {
    // Verifica permissão
    if (!this.canCallSubagent(name)) {
      const error = `⛔ ${this.name} NÃO tem permissão para chamar subagente: ${name}`;
      console.log(error);
      this.log(error);
      throw new Error(`Permissão negada: ${this.name} não pode chamar subagente ${name}`);
    }
  ...
  /**
   * Chama uma ferramenta (com validação de permissão)
   */
  async useTool(toolName, params) {
    // Verifica permissão
    if (!this.canUseTool(toolName)) {
      const error = `⛔ ${this.name} NÃO tem permissão para usar tool: ${toolName}`;
      console.log(error);
      this.log(error);
      throw new Error(`Permissão negada: ${this.name} não pode usar tool ${toolName}`);
    }
